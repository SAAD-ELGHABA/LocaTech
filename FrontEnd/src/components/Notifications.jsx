  ...
  return (
    ...
    >
      <motion.div
        ...
        className="fixed top-0 right-0 lg:w-[30%] w-[90%] bg-white h-full shadow-lg z-50"
            ...
            {notifications?.length > 0 ? (
              notifications?.map((n) => (
                <Link
                  to={n?.data?.link}
                  key={n?.id}
                  className={`flex items-center space-x-5 mx-3 my-1 p-2 hover:bg-red-100 rounded `}
                >
                  <div className="bg-yellow-500 text-white w-14 flex items-center justify-center h-14">
